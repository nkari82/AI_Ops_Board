...
export default function SettingsPage() {
  ...
  const [crawlHealth, setCrawlHealth] = useState<{
    status: string;
    timestamp: string;
  ...
  const refreshOpsSnapshot = React.useCallback(async () => {
    const [metricsData, healthData, crawlHealthData] = await Promise.all([
      fetchAdminMetricsApi(),
      fetchHealthDetailedApi(),
      fetchCrawlHealthApi(),
    ]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        const nextStatus = String(data.status || "UNKNOWN");
        setYtTaskStatus(nextStatus);
        setYtPollingErrorCount(0);
        pollingErrorCountRef.current = 0;
        setYtPollingIntervalMs(3000);
        pollingIntervalMsRef.current = 3000;

        if (["SUCCESS", "FAILURE", "REVOKED"].includes(nextStatus)) {
          setYtPolling(false);
          if (nextStatus === "SUCCESS") {
            ...
            setYtStatusMessage(`작업 종료 상태: ${nextStatus}`);
          }
          await refreshOpsSnapshot();
          return;
        }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4">
          <div>
            ...
            <p className="text-sm text-slate-600">운영 상태, 수집 품질, 크롤 작업 실행/추적을 관리합니다.</p>
          </div>
      ...
      <main className="mx-auto max-w-7xl space-y-4 px-4 py-6">
        <section className="grid gap-3 md:grid-cols-2">
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div className="grid gap-2 md:grid-cols-[180px_1fr_auto]">
              ...
              />
              <Button onClick={handleLlmTest}>LLM 테스트 실행</Button>
            </div>
            <div className="mt-2 text-xs text-slate-600">status: <span className="font-semibold">{llmStatus || "-"}</span> / provider: <span className="font-semibold">{llmProvider}</span></div>
            {llmResponse && (
              <pre className="mt-2 max-h-32 overflow-auto rounded-lg border border-slate-200 bg-slate-50 p-2 text-xs text-slate-700">{llmResponse}</pre>
            )}
          </div>

          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            <div className="mb-2 text-sm font-semibold text-slate-700">웹 크롤링 Health</div>
            <div className="mb-2 text-xs text-slate-500">overall: <span className="font-semibold">{crawlHealth?.status ?? "unknown"}</span> / updated: {formatKoreanDateTime(crawlHealth?.timestamp)}</div>
            <div className="space-y-1 text-sm text-slate-700">
              {crawlHealth?.sources ? (
                Object.entries(crawlHealth.sources).map(([source, info]) => (
                  <div key={source} className="flex items-center justify-between gap-2 rounded border border-slate-200 bg-slate-50 px-2 py-1">
                    <div>
                      <div className="font-medium capitalize">{source}</div>
                      <div className="text-xs text-slate-500">{info.detail || "-"}</div>
                    </div>
                    <div className="text-xs font-semibold">{info.status}</div>
                  </div>
                ))
              ) : (
                <div className="text-xs text-slate-500">크롤링 헬스 데이터가 없습니다.</div>
              )}
            </div>
            <div className="mt-2">
              <Button size="sm" variant="outline" onClick={() => refreshOpsSnapshot().catch((e) => setStatus(`스냅샷 갱신 실패: ${String(e)}`))}>
                크롤링 헬스 갱신
              </Button>
            </div>
          </div>
        ...
        {status && <div className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-700">{status}</div>}
      </main>
    </div>
  );
}
